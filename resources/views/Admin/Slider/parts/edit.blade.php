...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('sliders.update',$slider->id)}}">
    ...
    <div class="row mt-0">
        ...
        <div class=" mb-2 fv-row col-sm-6 mt-0 product_show " style="{{$slider->type != 'brand'?'display:none':''}}" id="category">
            <!--begin::Label-->
            <label class="d-flex align-items-center fs-6 fw-bold form-label ">
            ...
            <select class="form-control " id="category_change" name="category_id">
                ...
                @foreach($categories as $category)
                    <option value="{{$category->id}}"  {{$category->id == $slider->category_id && $slider->type == 'brand'?'selected':''}}>{{$category->name}}</option>
                @endforeach
            </select>
        </div>
        <div class=" mb-2 fv-row col-sm-6 mt-0 product_show " style="{{$slider->type != 'brand'?'display:none':''}}"  id="brand">
            <!--begin::Label-->
            <label class="d-flex align-items-center fs-6 fw-bold form-label ">
...
{{--                @endforeach--}}
                @if($brands)
                    @foreach($brands->categoryBrands as $brand)
                        <option {{$brand->brand->id == $slider->product_id && $slider->type == 'brand' ? 'selected':''}} value="{{$brand->brand->id}}" >{{$brand->brand->name}}</option>
                    @endforeach
                @endif
            </select>
        </div>
